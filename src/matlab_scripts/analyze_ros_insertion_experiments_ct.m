...
%% Plot the group statistics per day
error_columns = expmt_results.Properties.VariableNames( ...
    strcmp(expmt_results.Properties.VariableNames, "RMSE") ...
    | endsWith(expmt_results.Properties.VariableNames, "_Error") ...
);
error_columns = error_columns( ~( ...
    ...
    | strcmp(error_columns, "Max_Error") ...
));
...
for expmt_day = expmt_days'
    ...
    % plot
    fig_error_stats = figure(3);
    clf(fig_error_stats);
    fig_error_stats.Position = [100, 100, 800, 600];
    ...
    for plot_i = 1:numel(error_columns)
        ...
        bp = boxplot(...
            expmt_results{row_mask, error_columns{plot_i}}, ...
            expmt_results.insertion_depth(row_mask)...
        );
        set(bp, 'LineWidth', 2);
        title(strrep(error_columns{plot_i}, "_", " "))
    ...
    savefigas( ...
        fig_error_stats, ...
        fullfile(odir, sprintf("%s_ct_needle_shape_results", string(expmt_day))), ...
        'Verbose', true ...
    )
end
